...
def clean_images_bin(bin_img_path):
	file_list_bin = os.listdir(bin_img_path)
	for file_name in file_list_bin:
		if '.png' in file_name:
			os.remove(file_name)

def clean_images_class(class_img_path):
    file_list_class = os.listdir(class_img_path)
    for file_name in file_list_class:
    	if '.png' in file_name:
    		os.remove(file_name)
...
def main(args):
    if os.path.isdir(args.bin_img_path) and os.path.isdir(args.class_img_path):
        os.system("rm -rf " + args.bin_img_path)
        os.system("rm -rf " + args.class_img_path)
        os.mkdir(args.bin_img_path)
        os.mkdir(args.class_img_path)
    else:
        os.mkdir(args.bin_img_path)
        os.mkdir(args.class_img_path)
    ...
    out = cv2.VideoWriter(args.file_video_output, fourcc, fps , (640,480))
    ...
    coordinates = []
    position = []
    file = open(args.file_txt_output, "w")
    while True:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
      '--file_video_output',
      default= "public/uploads",
      help= "Path to the output avi file"
      )

    parser.add_argument(
      '--bin_img_path',
      default= "public/uploads",
      help= "Path to the binary image files"
      )

    parser.add_argument(
      '--class_img_path',
      default= "public/uploads",
      help= "Path to the classified image files"
      )
